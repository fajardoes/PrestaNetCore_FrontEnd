import { updateCollectionChannelType } from '@/core/api/collection-channels/collection-channels-api'
...
export const updateCollectionChannelTypeAction = async (
  channelTypeId: string,
  payload: UpsertCollectionChannelTypeRequest,
): Promise<ApiResult<CollectionChannelTypeResponse>> => {
  try {
    const data = await updateCollectionChannelType(channelTypeId, payload)
    return { success: true, data }
  } catch (error) {
    return toApiError(error, 'No fue posible actualizar el tipo de canal.')
  }
}
