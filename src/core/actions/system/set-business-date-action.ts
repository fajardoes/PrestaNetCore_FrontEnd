import { setBusinessDate } from '@/core/api/system/business-date-api'
...
export const setBusinessDateAction = async (
  payload: SetBusinessDateRequestDto,
): Promise<ApiResult<BusinessDateStateDto>> => {
  try {
    const result = await setBusinessDate(payload)
    ...
    return toApiError(
      error,
      'No fue posible actualizar la fecha operativa.',
    )
  }
}
