import { updateHolidayStatus } from '@/core/api/organization/holidays-api'
...
export const updateHolidayStatusAction = async (
  id: string,
  payload: HolidayStatusUpdateDto,
): Promise<ApiResult<HolidayDetailDto>> => {
  try {
    const result = await updateHolidayStatus(id, payload)
    ...
    return toApiError(error, 'No fue posible actualizar el estado del feriado.')
  }
}
